...
import numpy as np
import onnxruntime
from alfred.utils.file_io import ImageSourceIter, get_next_frame
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--imgpath', type=str, default='images/3.jpg', help="image path")
    parser.add_argument('-m', '--modelpath', type=str, default='weights/ppmattingv2_stdc1_human_480x640.onnx', help="image path")
    ...
    iter = ImageSourceIter(args.imgpath)
    while iter.ok:
        img = get_next_frame(iter)
        dstimg = segmentor.detect(img)
        iter.show('aa', dstimg)
        iter.waitKey()
